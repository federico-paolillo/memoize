...
namespace Memoization.Tests
{
	public class Memoizer_1_Test
    {
		...
		{
			//Arrange
			var fnSpy = new Mock<Func<string, string>>();
			...
			fnSpy.Verify(f => f(It.IsAny<string>()), Times.Once());

			//The comparer was called twice, the first time is excluded because there are no previous arguments
			comparerMock.Verify(c => c.Equals(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(2));
		}
	}
}
