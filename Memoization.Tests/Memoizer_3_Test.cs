...
namespace Memoization.Tests
{
	public class Memoizer_3_Test
    {
		...
		{
			//Arrange
			var fnSpy = new Mock<Func<string, string, string, string>>();
			...
			//Act
			memoizedFn.Call("ayYyy", "ayYyy", "ayYyy");
			memoizedFn.Call("ayYYyy", "ayYYyy", "ayYYyy");
			memoizedFn.Call("ayYYYy", "ayYYYy", "ayYYYy");

			//Assert

			//Function is called only once because the comparer returns always 'true'
			fnSpy.Verify(f => f(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()), Times.Once());

			//The comparer was called twice, the first time is excluded because there are no previous arguments
			comparerMock.Verify(c => c.Equals(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(6));
		}
	}
}
