...
namespace Memoization.Tests
{
	public class Memoizer_4_Test
    {
		...
		public void Call_invokes_original_function_when_arguments_changes()
		{
			...
			memoizedFn.Call(0, 1, 2, 3);
			memoizedFn.Call(0, 1, 2, 3);
			memoizedFn.Call(10, 11, 12, 13);
			memoizedFn.Call(10, 11, 12, 13);

			//Assert
			fnSpy.Verify(f => f(0, 1, 2, 3), Times.Once());
			fnSpy.Verify(f => f(10, 11, 12, 13), Times.Once());
		}
		...
		public void Converting_to_Func_implicitly_returns_a_reference_the_Call_method()
		{
			...
			Func<int, int, int, int, int> asFunc = memoizedFn;

			//Act
			asFunc.Invoke(0, 1, 2, 3);
			asFunc.Invoke(0, 1, 2, 3);
			asFunc.Invoke(0, 1, 2, 3);
		...
		public void Calling_reset_will_clear_all_memoized_information()
		{
			...
			//Assert
			fnSpy.Verify(f => f(0, 1, 2, 3), Times.Exactly(2));
		}
		...
		{
			//Arrange
			var fnSpy = new Mock<Func<string, string, string, string, string>>();
			...
			//Act
			memoizedFn.Call("ayYyy", "ayYyy", "ayYyy", "ayYyy");
			memoizedFn.Call("ayYYyy", "ayYYyy", "ayYYyy", "ayYYyy");
			memoizedFn.Call("ayYYYy", "ayYYYy", "ayYYYy", "ayYYYy");

			//Assert

			//Function is called only once because the comparer returns always 'true'
			fnSpy.Verify(f => f(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()), Times.Once());

			//The comparer was called twice, the first time is excluded because there are no previous arguments
			comparerMock.Verify(c => c.Equals(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(8));
		}
	}
}
