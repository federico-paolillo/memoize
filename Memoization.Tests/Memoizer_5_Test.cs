...
{
	public class Memoizer_5_Test
    {
        [Fact]
        public void Call_invokes_original_function_the_first_time()
        {
			//Arrange
			var fnSpy = new Mock<Func<int, int, int, int, int, int>>();

			var memoizedFn = Memoizer.Memoize(fnSpy.Object);

			//Act
			memoizedFn.Call(0, 1, 2, 3, 4);

			//Assert
			fnSpy.Verify(f => f(0, 1, 2, 3, 4), Times.Once());
        }
		...
		public void Call_invokes_original_function_when_arguments_changes()
		{
			...
			memoizedFn.Call(0, 1, 2, 3, 4);
			memoizedFn.Call(0, 1, 2, 3, 4);
			memoizedFn.Call(10, 11, 12, 13, 14);
			memoizedFn.Call(10, 11, 12, 13, 14);

			//Assert
			fnSpy.Verify(f => f(0, 1, 2, 3, 4), Times.Once());
			fnSpy.Verify(f => f(10, 11, 12, 13, 14), Times.Once());
		}
		...
		{
			//Arrange
			var fnSpy = new Mock<Func<object, object, object, object, object, int>>();

			var memoizedFn = Memoizer.Memoize(fnSpy.Object);

			//Act, call the function a few times
			memoizedFn.Call(null, null, null, null, null);
			memoizedFn.Call(null, null, null, null, null);
			memoizedFn.Call(null, null, null, null, null);

			//Assert
			fnSpy.Verify(f => f(null, null, null, null, null), Times.Once());
		}
		...
		public void Converting_to_Func_implicitly_returns_a_reference_the_Call_method()
		{
			...
			Func<int, int, int, int, int, int> asFunc = memoizedFn;

			//Act
			asFunc.Invoke(0, 1, 2, 3, 4);
			asFunc.Invoke(0, 1, 2, 3, 4);
			asFunc.Invoke(0, 1, 2, 3, 4);
		...
		public void Calling_reset_will_clear_all_memoized_information()
		{
			...
			//Assert
			fnSpy.Verify(f => f(0, 1, 2, 3, 4), Times.Exactly(2));
		}
		...
		{
			//Arrange
			var fnSpy = new Mock<Func<string, string, string, string, string, string>>();
			...
			//Act
			memoizedFn.Call("ayYyy", "ayYyy", "ayYyy", "ayYyy", "ayYyy");
			memoizedFn.Call("ayYYyy", "ayYYyy", "ayYYyy", "ayYYyy", "ayYYyy");
			memoizedFn.Call("ayYYYy", "ayYYYy", "ayYYYy", "ayYYYy", "ayYYYy");

			//Assert

			//Function is called only once because the comparer returns always 'true'
			fnSpy.Verify(f => f(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()), Times.Once());

			//The comparer was called twice, the first time is excluded because there are no previous arguments
			comparerMock.Verify(c => c.Equals(It.IsAny<string>(), It.IsAny<string>()), Times.Exactly(10));
		}
	}
}
